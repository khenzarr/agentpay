...
| Circle Wallets (Developer-Controlled / Programmable) | **CURRENT_VERIFIED (wallet creation + metadata read only)** | Founder-run server-only flow verified: entity secret registration succeeded, readiness passed (with `CIRCLE_TESTNET_BLOCKCHAIN=ARC-TESTNET`, `CIRCLE_WALLETS_DRY_RUN=true`, redacted secrets, and no live mutation), live wallet creation on `ARC-TESTNET` succeeded via `npm run circle:wallets:create:arc`, and non-mutating wallet metadata read succeeded via `npm run circle:wallets:get-wallet`; proof: `walletSetId=70d4bdf1-74a3-5098-8b37-5c573641e764`, `walletId=d99113e2-2e24-5d3f-ab6d-7b8c49367566`, `walletAddress/address=0x156c37d9a28b67588720116a13fba1ff7a5275f8`, `accountType=EOA`, `custodyType=DEVELOPER`, `state=LIVE`; `.env.circle.local`/`.circle-recovery` git-ignored; no secrets committed | Claim allowed only for wallet creation and metadata read on ARC-TESTNET; signing/send/gasless/paymaster remain unverified |
...
- Demo/readiness docs updated to strict classification wording: App Kit Send is CURRENT_VERIFIED; Bridge/CCTP is CURRENT_VERIFIED; Gateway/Unified Balance is CURRENT_CODE_IMPLEMENTED_SPEND_ESTIMATE_VERIFIED (below CURRENT_VERIFIED pending live spend proof); Circle Wallets is CURRENT_VERIFIED (wallet creation + metadata read only); Paymaster remains NOT_CLAIMED.
...
- Current claim status is now: **CURRENT_VERIFIED (wallet creation + metadata read only)**
...
- Circle Wallets product claim: **CURRENT_VERIFIED (wallet creation + metadata read only)**.
...
- Internal readiness implementation state is **CURRENT_VERIFIED** for wallet creation + metadata read.
