...
| Circle Wallets (Developer-Controlled) | **CURRENT_VERIFIED (wallet creation + metadata read + message signing proof)** | Founder-run server-only proof captured: entity secret registration succeeded, readiness passed (`CIRCLE_TESTNET_BLOCKCHAIN=ARC-TESTNET`, `CIRCLE_WALLETS_DRY_RUN=true`, secrets redacted, no live mutation by readiness), `npm run circle:wallets:create:arc` created an ARC-TESTNET wallet, non-mutating `npm run circle:wallets:get-wallet` metadata read succeeded, and benign server-only message signing succeeded via `npm run circle:wallets:sign-message` (`walletId=d99113e2-2e24-5d3f-ab6d-7b8c49367566`, status `200`, signature captured); `.env.circle.local` + `.circle-recovery` git-ignored; no secrets committed | Claim allowed for wallet creation, metadata read, and message signing only; send/transfer/gasless/paymaster remain unverified |
...
- 7. Circle Developer-Controlled Wallet creation, metadata read, and message signing proof on ARC-TESTNET
...
- Current public/demo status: **CURRENT_VERIFIED (wallet creation + metadata read + message signing only)** with strict boundary; send/transfer/gasless/paymaster remain unverified.
...
Founder-run live Circle wallet creation proof and server-only benign message-signing proof were executed in this sprint; send/gasless/paymaster proof remains unverified.
